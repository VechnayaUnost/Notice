package by.darya.zdzitavetskaya.notice.common.constants;

import android.os.Message;

public class Constants {

    public static final String MESSAGE = Message.class.getSimpleName();

    public static final int CURRENT_NOTICE_TAB = 0;
    public static final int COMPLETED_NOTICE_TAB = 1;

    public static final String FIELD_NAME_DATE = "date";

    public static final String THEME_NAME = "AppTheme";
    public static final String THEME_ORANGE_NAME = "AppThemeOrange";
    public static final String THEME_BLACK_NAME = "AppThemeBlack";
}
